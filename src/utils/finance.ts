...
export function calculateFinancialSummary(
  ...
  const validCollections = members.length > 0
  ...
  if (members && members.length > 0) {
    ...
    todayRegistrationsCount = members.filter((m) => m.joinDate === today).length;
    ...
    const regFeeCols = validCollections.filter((c) => c.type === "Registration Fee");
    registrationRevenue = regFeeCols.length * regFee;
    
    const todayRegFeeCols = regFeeCols.filter((c) => c.timestamp.startsWith(today));
